...
def generate_train_pairs(config: OmegaConf):
    '''
        ...
        Given the trained AIAE model, generate pairs of images from the training set.
        The pairs are generated by finding the closest image in the anchor bank to each image in the training set.
        The anchor bank: training images from the original or augmented images.
    '''
    device = torch.device(
        'cuda:%d' % config.gpu_id if torch.cuda.is_available() else 'cpu')
    ...
    for i in range(len(to_be_matched_embeddings)):
        ...
        closest_dists.append(dist_matrix[i, closest_index])
