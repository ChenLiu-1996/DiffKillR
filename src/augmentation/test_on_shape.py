...
def test_on_shape(shape: str = 'square'):
    ...
    # Similar to Yale Blue but brighter.
    image[:, :, 0] = 120/255*generate_shape(shape=shape, random_seed=1)
    image[:, :, 1] = 160/255*generate_shape(shape=shape, random_seed=1)
    image[:, :, 2] = 255/255*generate_shape(shape=shape, random_seed=1)

    ax = fig.add_subplot(5, 6, 1)
    ax.imshow(image)
    ax.set_title('Input "cell"')
    ax.set_axis_off()
    ...
    for seed in range(5):
        ...
        line_segment1 = find_line_with_point_and_angle(image.shape, shape_centroid, rotation_angle)
        line_segment2 = find_line_with_point_and_angle(image.shape, shape_centroid, rotation_angle + 180)
        ax = fig.add_subplot(5, 6, seed + 20)
        ax.imshow(image_stretched)
        ax.plot(*line_segment1, color='white', linestyle=':')
        ax.plot(*line_segment2, color='white', linestyle=':')
    ...
    for seed in range(5):
        ...
        ax = fig.add_subplot(5, 6, seed + 26)
        ax.imshow(image_stretched)
...
def test_on_shape_with_label(shape: str = 'square'):
    ...
    fig.tight_layout(pad=2)
    fig.savefig('synthetic_test_with_label_%s.png' % shape)
...
if __name__ == '__main__':
    ...
    test_on_shape_with_label(shape=args['shape'])