...
def extract_fileID(fp: str):
    '''
    Extract the file ID from the file name.
    '../data/DB-0001_normal_2_H-10_W574_patch_96x96.png' -> 'DB-0001_normal_2'
    '''
    file_name = os.path.basename(fp)
    h_w = re.findall('H(-?\d+)_W(-?\d+)', file_name)
    h_w_str = '_H' + h_w[0][0] + '_W' + h_w[0][1]
    index = file_name.index(h_w_str)
...
def main():
    ...
    argparser.add_argument('--organ', type=str, default='Normal')
    ...
    if args.organ in GLySAC_Organ2FileID:
        file_ids = GLySAC_Organ2FileID[args.organ]['train']
    ...
    print('file_ids:', file_ids, 'num_patches_per_image:', num_patches_per_image)
    ...
    for i in range(len(image_path_list)):
        ...
        file_id = extract_fileID(image_path_list[i])
        print(file_id, '\n', image_path_list[i])
    ...
    augmented_folder = f'../data/{percentage:.3f}_{args.organ}_m{multiplier}_GLySACTrainData_augmented_patch_{augmented_patch_size}x{augmented_patch_size}/'
    # Overwrite 'GLySAC_data.yaml' config file so models can find the data
    if args.detection == 'BlobLocalization':
        test_folder = f'../data/GLySACTestData_BlobLocalization_patch_{augmented_patch_size}x{augmented_patch_size}'
    ...
    conf = OmegaConf.create(
        {
            ...
            'groudtruth_folder': '../external_data/GLySAC/Test/masks',
            'log_folder': '../logs/',
            'percentage': percentage,
            'organ': args.organ,
    ...
    OmegaConf.save(conf, './config/GLySAC_data.yaml')
