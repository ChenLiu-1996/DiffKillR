...
def preProcess_train(x, y, args):
    ...
    input_data, label = input_data.permute(0,3,1,2).contiguous().float().cuda(), label.permute(0,3,1,2).contiguous().float().cuda()
    ...
    if args.mode in ('train_final_stage'):
        ...
        if args.mode == 'train_final_stage':
            ...
            return input_data, label, edge_data
        return input_data,label/255,edge_data/255

    return input_data,label/255
...
def trainer_selfsupervised(EPOCH,args,model, loss_func, optimizer):
    for epoch in range(EPOCH):
        ...
        for x, y in tqdm(loader.train_loader, desc='training'):
