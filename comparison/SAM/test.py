...
sam = sam_model_registry["default"](checkpoint=os.path.join('./checkpoints/', "sam_vit_h_4b8939.pth")).to(device)
...
for batch_idx, (X_batch, y_batch, *rest) in enumerate(tqdm(valloader)):
    ...
    assert len(label_sam.shape) == 2
    label_sam = label_sam.astype(int)[None, None, ...]
